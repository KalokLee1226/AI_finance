<template>
  <div class="dash-root min-h-screen font-sans bg-white text-slate-900 flex flex-col">
    <!-- Main Content -->
    <main class="flex-1 flex flex-col overflow-hidden relative">
      <!-- Header -->
      <header
        class="h-28 bg-white/92 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-20"
      >
        <div class="flex flex-col space-y-3 max-w-[60%]">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <button
              ...
              :class="[
                'px-3 py-1.5 rounded-full text-xs border flex items-center gap-2',
                activeComm === key
            ...
            <button
              type="button"
              class="px-3 py-1.5 rounded-full text-xs border border-dashed border-slate-400 text-slate-600 hover:bg-slate-900 hover:text-white"
        ...
        <div class="flex items-center gap-4">
          <div
            class="inline-flex rounded-full border border-slate-300 bg-white overflow-hidden text-xs"
          >
            <button
              type="button"
              class="px-3 py-1"
          ...
          <button
            type="button"
            class="px-3 py-1.5 rounded-full border border-slate-300 text-xs text-slate-700 hover:bg-slate-900 hover:text-white"
            @click="fetchData"
          >
      ...
      <div class="flex-1 overflow-auto p-8 section-scroll bg-white space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[7fr_3fr] gap-8">
          <!-- Chart Card -->
          <div
            class="bg-white p-1 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden group hover:shadow-lg hover:border-slate-300 transition-all"
          >
          ...
          <div
            class="bg-white rounded-2xl shadow-sm border border-slate-200 hover:shadow-lg hover:border-slate-300 transition-all flex flex-col relative overflow-hidden"
          >
            ...
            <div class="px-8 py-6 border-b border-slate-100 flex flex-col gap-4 bg-slate-50/50">
              ...
              <div class="flex flex-wrap items-center gap-4 ml-9 text-xs text-slate-600">
                <div class="flex items-center gap-2">
                  ...
                  <select
                    v-model="personaKey"
                    class="border border-slate-300 rounded px-2 py-1 text-xs bg-white text-slate-700"
                  >
                  ...
                  <input
                    ...
                    :placeholder="
                      lang === 'en'
                        ...
                        : '自定义人物名称，例如：XX 股神'
                    "
                    class="flex-1 min-w-[120px] px-2 py-1 rounded border border-slate-300 bg-white text-xs"
                  />
                  <input
                    ...
                    :placeholder="
                      lang === 'en'
                        ...
                        : '简要描述其性格、风险偏好和投资风格'
                    "
                    class="flex-[2] min-w-[200px] px-2 py-1 rounded border border-slate-300 bg-white text-xs"
                  />
                </div>
            ...
            <div class="p-8 min-h-[300px] relative">
              ...
              >
                <div class="relative w-24 h-24 mb-6">
                  <div
                    class="absolute inset-0 rounded-full border-t-4 border-indigo-500 animate-[spin_1s_linear_infinite]"
                  ></div>
                  <div
                    class="absolute inset-2 rounded-full border-r-4 border-purple-500 animate-[spin_1.5s_linear_infinite_reverse]"
                  ></div>
                  <div
                    class="absolute inset-4 rounded-full border-b-4 border-pink-500 animate-[spin_2s_linear_infinite]"
                  ></div>
                  <div class="absolute inset-0 flex items-center justify-center text-xl">
                    🧠
                  </div>
                </div>
                <p class="text-indigo-800 font-bold text-lg animate-pulse">
                  DeepSeek 正在进行交叉推演...
                </p>
              ...
              <div
                ...
                class="prose prose-slate max-w-none w-full markdown-body bg-slate-50/80 p-8 rounded-xl border border-slate-200"
                v-html="reportHtml"
              ></div>
        ...
        <!-- 第二行：专家观点 + Latest News（均具备翻译功能） -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
          <!-- 专家观点 -->
          <div
            class="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden hover:shadow-lg hover:border-slate-300 transition-all"
          >
            <div
            ...
            <div class="flex-1 p-4 space-y-3 custom-scrollbar max-h-80 overflow-y-auto">
              ...
              <div
                ...
                :key="idx"
                class="p-3 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-colors cursor-pointer group"
                @click="op.link && window.open(op.link, '_blank')"
              >
                <div class="flex justify-between items-center mb-1">
                  <span class="text-xs font-semibold text-slate-500 group-hover:text-slate-200">
                    {{ op.source }}
                  </span>
                ...
                <div class="flex justify-between items-center mb-1">
                  ...
                  >
                    {{ expertTranslations[idx] || (op.title || '未命名观点') }}
                  </p>
                  <button
                    type="button"
                    class="text-[10px] px-2 py-0.5 rounded-full border border-slate-300 bg-white/70 text-slate-700 hover:bg-slate-900 hover:text-white"
                ...
                <p
                  v-if="op.summary"
                  class="text-[11px] text-slate-500 group-hover:text-slate-200 leading-relaxed line-clamp-3"
                >
          ...
          >
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              ...
              <div
                ...
                :key="idx"
                class="p-3.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-colors group"
              >
                <div class="flex justify-between items-start mb-1">
                  <p
                    class="text-sm text-slate-900 font-semibold leading-relaxed group-hover:text-white flex-1 mr-2"
                  >
                    {{ newsTranslations[idx] || item.title }}
                  </p>
                  <button
                    type="button"
                    class="text-[10px] px-2 py-0.5 rounded-full border border-slate-300 bg-white/70 text-slate-700 hover:bg-slate-900 hover:text-white shrink-0"
                ...
                <div class="flex justify-between items-center mt-2">
                  <span
                    class="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-white/80 text-slate-700 border border-slate-300 uppercase tracking-[0.16em]"
                  >
                    {{ item.source }}
                  </span>
...
const translateExpertItem = async (op, idx) => {
  ...
  try {
    ...
    if (res?.translated) {
      expertTranslations.value = { ...expertTranslations.value, [idx]: res.translated }
    }
  } catch (err) {
